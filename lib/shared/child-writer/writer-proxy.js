    ...
    ipc.connectTo(process.env.IPC_SERVER_ID, () => {
      ...
      ipc.of[process.env.IPC_SERVER_ID].on('connect', () => {
        ...
        const emitMessage = (data) => {
          ...
          // separate lines (ignore new lines inside double quotes, which happen
          // inside stacktrace property of error messages), and only emit a
          // "message" event for the last one.
          const object = _.chain(data.toString())
            .split(/((?:[^\n"']|"[^"]*"|'[^']*')+)/)
            .reject((string) => {
