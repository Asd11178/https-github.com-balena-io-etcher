...
SDK.Scanner = class Scanner extends EventEmitter {
  ...
  /**
   * @summary Make a device accessible
   * @public
   * @param {Object} device - device
   * @returns {Promise}
   * @example
   * scanner.makeDeviceAccessible({ ... }).then(() => {
   *   console.log('Accessible!')
   * })
   */
  makeDeviceAccessible (device) {
    const instance = this.adapters.get(device.adaptor)
    if (!instance || !instance.makeDeviceAccessible) {
      return Bluebird.resolve()
    }

    return instance.makeDeviceAccessible(device)
  }
}
