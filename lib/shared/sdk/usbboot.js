...
exports.scan = () => {
  ...
  /* eslint-enable lodash/prefer-lodash-method */
    ...
    return usbUtils.getDeviceName(device).then((name) => {
      userFriendlyDeviceObject.name = name
      return userFriendlyDeviceObject
    }).finally(_.bind(device.close, device))
  })
}

const writeEndpoint = (device, deviceInterfaceId, buffer) => {
  return usbUtils.findEndpoint(device.instance, deviceInterfaceId, device.outEndpoint).then((endpoint) => {
...
const writeBootCode = (device, bootCodeBuffer) => {
  ...
  return writeEndpoint(device, device.interface, bootMessageBuffer).delay(USB_OPERATION_DELAY_MS).then(() => {
    debug('Writing boot code buffer to out endpoint')
    return writeEndpoint(device, device.interface, bootCodeBuffer)
