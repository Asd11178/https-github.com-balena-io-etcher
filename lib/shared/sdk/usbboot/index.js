...
const bootMessage = require('./boot-message')
 ...
 * @constant
 */
const USBBOOT_BOOTCODE_RETURN_CODE_SUCCESS = 0
 ...
 *   return writeBufferToEndpoint(devices[0], devices[0].interfaces[0].endpoints[1], Buffer.alloc(1)).then(() => {
 *     console.log('Done!')
 *   })
 * })
 */
const writeBufferToEndpoint = (device, endpoint, buffer) => {
 ...
 * @example
 * usbboot.scan().then((devices) => {
 *   return readBufferFromDevice(devices[0], 4).then((data) => {
 *     console.log(data.readInt32BE())
 *   })
 * })
 */
const readBufferFromDevice = (device, bytesToRead) => {
  ...
  return usbUtils.findInterface(device.instance, device.interface).then((deviceInterface) => {
    ...
    return Bluebird.try(() => {
      if (serialNumberIndex === USB_DESCRIPTOR_NULL_INDEX) {
        ...
        const bootMessageBuffer = bootMessage.create(options.bootcode.length)

        return usbUtils.findEndpoint(device.instance, device.interface, device.outEndpoint).tap((endpoint) => {
          debug('Writing boot message buffer to out endpoint')
          return writeBufferToEndpoint(device, endpoint, bootMessageBuffer)
        }).then((endpoint) => {
          debug('Writing boot code buffer to out endpoint')
          return writeBufferToEndpoint(device, endpoint, options.bootcode)
        }).then(() => {
          debug('Reading return code from device')
          return readBufferFromDevice(device, USBBOOT_RETURN_CODE_LENGTH)
        }).then((data) => {
          const returnCode = data.readInt32BE()
          debug(`Received return code: ${returnCode}`)

          if (returnCode !== USBBOOT_BOOTCODE_RETURN_CODE_SUCCESS) {
