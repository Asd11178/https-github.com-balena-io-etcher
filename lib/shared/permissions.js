 ...
 *
 * @example
 * const result = doubleQuoteString('foo');
 */
const doubleQuoteString = (string) => {
  return `"${string}"`;
};
...
exports.getEnvironmentCommandPrefix = (environment) => {
  ...
  const argv = _.flatMap(environment, (value, key) => {
    ...
    if (isWindows) {
      return [ 'set', commandJoin([ `${key}=${value}` ]), '&&' ];
    }

    return [ `${key}=${commandJoin([ value ])}` ];
  });
 ...
 *
 * // Windows
 * [ 'cmd.exe', '/c', '"set "ELECTRON_RUN_AS_NODE=1" && call foo baz"' ]
 *
 * // Unix
...
exports.buildElevateCommand = (command, environment = {}) => {
  const prefixedCommand = _.concat(
    ...
    _.map(command, (string) => {
      if (/[&;%^]/g.test(string)) {
        return doubleQuoteString(string);
      }

      return commandJoin([ string ]);
    })
  );
  ...
  if (os.platform() === 'win32') {
    return [
      'cmd.exe',
      '/c',
      doubleQuoteString(_.join(prefixedCommand, ' '))
    ];
  }
