...
let targetLedsMap = null

try {
  targetLedsMap = require('../modules/target-leds-map.ts')
} catch (err) {
  // Leds not available on this system
}
...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      if (settings.get('usbLedsMap')) {
        // Turn on all leds connected drives leds
        _.each(drives, (drive) => {
          if (drive.devicePath) {
            const driveLed = targetLedsMap.getDriveLed(drive.devicePath)
            driveLed.setColor(0, 1, 0)
          }
        })

        // Turn off all disconnected drives leds
        _.each(targetLedsMap.getLedTargetMap(), (driveLed, drivePath) => {
          const disconnected = _.find(drives, (drive) => _.includes(drive.devicePath, drivePath))
          if (!disconnected) {
            driveLed.setColor(0, 0, 0)
          }
        })
      }
    ...
    case ACTIONS.SET_FLASH_STATE: {
      ...
      if (action.data.verifying && settings.get('usbLedsMap')) {
        // Blink all validating leds purple
        _.each(action.data.destinations, (drive) => {
          const driveLed = targetLedsMap.getDriveLed(drive.devicePath)
          if (driveLed.currentAnimationName !== 'blink-purple') {
            driveLed.setAnimation('blink-purple')
          }
        })
      }
