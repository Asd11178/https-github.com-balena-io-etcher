  ...
  return new Bluebird((resolve, reject) => {
    ...
    ipc.server.on('fail', ({ device, error }) => {
      if (settings.get('usbLedsMap')) {
        ...
        const driveLed = targetLedsMap.getDriveLed(device.devicePath)
        driveLed.setAnimation()
        driveLed.setColor(1, 0, 0)
      }
    ...
    ipc.server.on('ready', (data, socket) => {
      if (settings.get('usbLedsMap')) {
        ...
        _.each(drives, (drive) => {
          ...
          driveLed.setAnimation('blink-orange')
        })
      }
    ...
    ipc.server.on('start', () => {
      ...
      permissions.elevateCommand(argv, {
        ...
        if (settings.get('usbLedsMap')) {
          // Turn red leds for failed flashes
          _.each(flashResults.results.errors, (drive) => {
            const driveLed = targetLedsMap.getDriveLed(drive.devicePath)
            driveLed.setAnimation()
            driveLed.setColor(1, 0, 0)
          })

          // Turn green leds for successful flashes
          _.each(flashResults.results.devices, (drive) => {
            const failed = _.find(flashResults.results.errors, (error) => _.includes(error.devicePath, drive.devicePath))
            if (!failed) {
              const driveLed = targetLedsMap.getDriveLed(drive.devicePath)
              driveLed.setAnimation()
