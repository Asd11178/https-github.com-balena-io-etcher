...
const writeAndValidate = async (source, destinations, verify, trim, onProgress, onFail) => {
  ...
  const result = {
    ...
    devices: _.map(destinations, 'drive'),
    errors: []
  }
  for (const [ destination, error ] of failures) {
    error.device = destination.drive.device
    error.devicePath = destination.drive.devicePath
...
ipc.connectTo(IPC_SERVER_ID, () => {
  ...
  ipc.of[IPC_SERVER_ID].on('write', async (options) => {
    ...
    const onProgress = (state) => {
      const enhancedState = _.merge({}, state, options)
      ipc.of[IPC_SERVER_ID].emit('state', enhancedState)
    }
